...
import { Sec, Callout, Read, Rounds } from './ui'
...
export function Trend() {
  ...
  const maxN = Math.max(...byRound.map((r) => r.n), 1)
  ...
  return (
    <>
      ...
      <Sec n="01" title="월별 추이 · 낙찰가율과 낙찰률" sub="12개월">
        <Read tone="warn" title="이 그래프만 용도가 다릅니다">
          지금 가진 월별 자료는 <b>{MONTHLY_USAGE}</b> 기준입니다(기준 물건이 일반상업이라 그렇게 뽑혔습니다).
          <b> 주택 4종 월별 자료를 받으면 갈아끼웁니다.</b> 아래 용도별 비교부터는 전부 주택 4종입니다.
        </Read>
        <div className="chart">
          <DualLine
            labels={labels}
      ...
      <Sec n="02" title="용도별 비교 · 서울 전체 vs 화곡동" sub={`${MARKET_PERIOD} · 주택 4종`}>
        <Callout title="연립/다세대는 화곡동이 더 잘 팔리는데(26.49% vs 24.22%) 더 싸게 팔립니다(70.00% vs 75.20%)">
          응찰자는 오히려 적습니다. 값이 충분히 내려가야 팔리는 구조입니다.
        </Callout>
        <div className="scroll">
          <table>
            ...
            <tbody>
              <tr className="dim"><td /><td className="num" colSpan={3}>서울 전체</td><td className="num" colSpan={3}>화곡동</td></tr>
              {HOUSING.map((u) => {
      ...
      <Sec n="03" title="올린 목록의 회차별 낙찰" sub={isEmpty ? undefined : `${rows.length}건`}>
        {isEmpty ? (
          <p className="dnote">목록을 올리면 회차별로 몇 건이 어떤 값에 팔렸는지 나옵니다.</p>
        ) : (
          <>
            <Rounds items={byRound.map((b) => ({
              round: `${b.round}회차 · ${(100 * 0.8 ** (b.round - 1)).toFixed(1)}%`,
              price: `${b.n}건`,
              sub: b.rate !== null ? `평균 낙찰가율 ${b.rate.toFixed(1)}%` : '—',
              tone: b.n === maxN ? 'hot' as const : undefined,
            }))} />
            <Read>
              회차는 <b>최저가 ÷ 감정가로 되짚어 센 값</b>이라 기일 순번과 다를 수 있습니다 — 그래서 감정가 대비 %를 같이 적었습니다.
              올린 목록에는 <b>매각된 물건만</b> 들어 있으므로 이 줄은 &lsquo;그 회차에서 팔린 물건 수&rsquo;입니다.
            </Read>
          </>
        )}
      </Sec>

      <Sec n="04" title="실거래 평당가 · 같이 볼 값" sub={`${MARKET_PERIOD} · 강서구 · 일반 매매`}>
        <div className="scroll">
          <table>
            <thead>
            ...
            <tbody>
              {DEALS.map((d) => (
                <tr key={d.usage} className={d.usage.includes('오피스텔') ? undefined : 'dim'}>
        ...
        <Read>
          실거래 자료도 <b>연립/다세대가 빠져 있습니다</b> — 통계 재출력을 요청해 둔 상태입니다. 지금은 오피스텔(주거용)만 대상과 겹칩니다.
        </Read>
      </Sec>
    </>
  )

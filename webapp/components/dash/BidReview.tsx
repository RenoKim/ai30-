    ...
    <>
      ...
      <section className="dcard">
        ...
        <div className="scroll">
          <table>
            <tbody>
              <tr><th>용도 · 지역</th><td>{picked.usageName} · {picked.district}</td></tr>
              <tr><th>특수권리</th><td>{picked.rights.length ? picked.rights.join(' · ') : '없음'}</td></tr>
              <tr><th>감정가</th><td>{won(picked.appraisalWon)}원</td></tr>
              <tr><th>최저가</th><td>{won(picked.minBidWon)}원</td></tr>
              <tr className="calc"><th>현재 회차</th>
                <td>{picked.roundNo}회차 ({(picked.roundNo ?? 1) - 1}회 유찰) · 감정가의 {picked.minToAppraisalPct}%</td></tr>
              <tr className="calc"><th>평당가 (감정가 기준)</th>
                <td>{myPyeong ? `${Math.round(myPyeong / 10_000).toLocaleString('ko-KR')}만원` : '면적이 없어 계산 못 함'}</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="dcard">
        <h3>비슷한 물건의 낙찰가 분포</h3>
        <p className="dsub">
          {picked.usageName} · {picked.roundNo}회차 · <b>{peers.length}건</b> · 가로는 낙찰가 ÷ 최저가
        </p>
        {ratios.length < 3 ? (
          <p className="dwarn">
            ⚠️ 같은 조건이 <b>{ratios.length}건</b>뿐입니다. 분포라고 부를 수 없습니다.
            목록을 더 올리면 표본이 늘어납니다.
          </p>
        ) : (
          <>
      ...
      <section className="dcard">
        <h3>입찰가 구간</h3>
        <p className="dsub">위 분포를 이 물건 최저가 {won(picked.minBidWon)}원에 적용한 값</p>
        <div className="scroll">
          <table>
            <thead>
            ...
            <tbody>
              {bands.map(([label, ratio, chance]) => (
                <tr key={label} className={label === '중위' ? 'on' : undefined}>
                  <td><b>{label}</b></td>
                  <td className="num">{ratio.toFixed(1)}%</td>
                  <td className="num"><b>{won((picked.minBidWon * ratio) / 100)}원</b></td>
                  <td className="num">{((picked.minBidWon * ratio) / picked.appraisalWon).toFixed(3).replace('0.', '') === '' ? '-' : `${(((picked.minBidWon * ratio) / 100 / picked.appraisalWon) * 100).toFixed(1)}%`}</td>
                  <td>{chance}</td>
                </tr>
              ))}
              <tr className="calc">
        ...
        <p className="concl">
          한 번 더 유찰되면 최저가가 <b>{won(picked.minBidWon - nextMinBid)}원</b> 내려갑니다.
        ...
        {deal && myPyeong ? (
          <>
            <div className="stat-row">
              <div><span className="k">이 물건 평당가 (감정가)</span>
                <span className="v">{Math.round(myPyeong / 10_000).toLocaleString('ko-KR')}<em>만원</em></span></div>
              <div><span className="k">인근 실거래 중위 평당가</span>
                <span className="v">{deal.medPyeong.toLocaleString('ko-KR')}<em>만원</em></span>
                <span className="d">{deal.usage} · {deal.count}건</span></div>
              <div><span className="k">감정가 ÷ 실거래</span>
                <span className={`v${myPyeong / 10_000 > deal.medPyeong ? ' alarm' : ''}`}>
                  {((myPyeong / 10_000 / deal.medPyeong) * 100).toFixed(0)}<em>%</em></span></div>
            </div>
            <p className="dnote">
              감정가가 실거래보다 {myPyeong / 10_000 > deal.medPyeong ? '높습니다' : '낮습니다'}.
              <b> 낙찰가율이 낮아 보여도 실거래 대비로는 다르게 읽힐 수 있습니다.</b>{' '}
              (감정가는 건물+대지권 합이라 평당가는 근사치입니다.)
            </p>
          </>
        ) : (
          <p className="dwarn">
            ⚠️ 이 용도의 실거래 자료가 없습니다. 지금 가진 실거래 통계는
            업무/상업시설과 오피스텔(주거용)뿐이라 <b>연립/다세대가 빠져 있습니다</b> —
            통계 재출력을 요청해 둔 상태입니다.
          </p>
        )}
      </section>
